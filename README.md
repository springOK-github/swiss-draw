...
- **対戦結果記録**
  ...
  - 統計の自動更新（勝点・勝数・敗数・消化試合数）
  - 対戦履歴の完全な記録
  
- **順位表示**
  - 勝点順の順位表を表示
  - タイブレーク（オポネント勝率、自己勝率、消化試合数）による順位付け
  - オポネント勝率（対戦相手の平均勝率）を自動計算
...
- 勝点内での順位: オポネント勝率 → 自己勝率 → 消化試合数（少ない方が優先）
- 過去に対戦した相手との再戦を自動回避

### オポネント勝率（Opponent Match Win Percentage）
...
3. **結果記録**: 「✅ 対戦結果の記録」で各対戦の結果を記録
   - 勝敗または引き分けを選択
   - プレイヤーIDを入力
4. **進行確認**: 「📊 ラウンド状況確認」で記録状況を確認
5. **次ラウンド**: すべての対戦結果が記録されたら、次のラウンドを開始

#### トーナメント終了
1. 「🏅 順位表示」で最終順位を確認
2. 必要に応じて「🔄 トーナメントのリセット」で新しいトーナメントを開始
...
- **🔄 トーナメントのリセット**: トーナメントを初期状態に戻す（全データ削除）
...
1. **プレイヤーシート**: プレイヤーマスタ
   - プレイヤーID、名前、勝点、勝数、敗数、消化試合数、参加状況、最終対戦日時
...
### Q. トーナメントを最初からやり直したい
A. カスタムメニューの「🔄 トーナメントのリセット」を実行してください。すべてのラウンドデータと対戦履歴が削除され、プレイヤーの統計がリセットされます。
