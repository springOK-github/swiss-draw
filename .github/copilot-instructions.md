...
このドキュメントは、AIエージェントが本リポジトリのメンテナンスや機能拡張を行う際の指針として利用できるように構成されています。作業に入る前に以下の原則を確認してください。

### 基本方針

- 既存のロック取得、シート操作、勝率計算のパターンを必ず尊重する
- UI から呼ばれる関数は例外処理とユーザー通知（`ui.alert`）を保持する
- シートの列構造や定数の整合性を壊さない
- 新規関数を追加する前に既存関数の再利用可否を検討する

### 変更前の確認事項

- 対象となるシート名・列名が `constants.js` と `REQUIRED_HEADERS` と一致しているか
- プレイヤーや対戦データを扱う処理でロック（`acquireLock`）が必要か
- 既存の勝点・勝率計算ロジック（`updatePlayerStats`、`updateAllOpponentWinRates`）に影響がないか

### コード編集時の必須チェック

- GAS の制約上、`try/finally` でロック解放しているかを確認
- スイス方式のマッチング（`matchPlayersSwiss`）で Map/Set を利用したキャッシュを壊していないか
- `promptPlayerId()` などの UI 補助関数を正しく利用しているか
- 日時の扱いは `Utilities.formatDate(... 'Asia/Tokyo')` を統一する

### ドキュメント更新のポイント

- README やユーザーフローを変更する場合は、カスタムメニューとシート構成の手順が最新か確認
- 本ガイド（`copilot-instructions.md`）に追記する場合は、AI エージェントが遵守すべきルールを明示的に記載する

### テスト・検証手順

- シート構造変更時は `setupSheets()` が期待どおりのヘッダーを生成するか確認
- マッチングロジックに変更を加えた場合は、`registerTestPlayers()` → `startNewRound()` の流れで実際にマッチングが作成されるか検証
- 対戦結果処理を編集した場合は、勝点・勝数・OMW% の更新が 想定どおりか `recordMatchResult()` と `updatePlayerStats()` を中心に追う

### 安全な作業手順の例

1. 目的の機能や不具合を特定し、関連ファイルと関数を確認する
2. 既存のガード（ロック、バリデーション、UI確認）が維持されるようにコードを変更
3. `test-utils.js` のテストプレイヤーを使って最低限の動作確認を行う
4. README やメニュー表示に影響する場合は、必要なドキュメント更新を行う
5. 変更内容と影響範囲をコミットメッセージに明確に残す
